...
function voice(who, big = false) {
  if (!audioCtx) return; const v = VOICES[CAST_VOICE[who % CAST_VOICE.length]], t = audioCtx.currentTime, dur = v.dur * (big ? 1.25 : 1);
  const out = audioCtx.createGain(); out.gain.setValueAtTime(0.0001, t); out.gain.exponentialRampToValueAtTime(big ? 0.55 : 0.4, t + 0.05); out.gain.setValueAtTime(big ? 0.55 : 0.4, t + dur * 0.5); out.gain.exponentialRampToValueAtTime(0.0001, t + dur);
  const f1 = audioCtx.createBiquadFilter(), f2 = audioCtx.createBiquadFilter(); f1.type = 'bandpass'; f1.frequency.value = v.f * 1.6; f1.Q.value = 2; f2.type = 'bandpass'; f2.frequency.value = v.f * 3.2; f2.Q.value = 3; // two formants: a rough "aah"
  const lfo = audioCtx.createOscillator(), lg = audioCtx.createGain(); lfo.frequency.value = v.wob; lg.gain.value = v.f * 0.05; lfo.start(t); lfo.stop(t + dur + 0.1);
  [1, 2, 3].forEach((h) => { const o = audioCtx.createOscillator(); o.type = h === 1 ? v.type : 'sine'; o.frequency.setValueAtTime(v.f * h * (big ? 1.12 : 1), t); o.frequency.exponentialRampToValueAtTime(v.slide * h, t + dur); lg.connect(o.frequency); const g = audioCtx.createGain(); g.gain.value = h === 1 ? 1 : 0.5 / h; o.connect(g); g.connect(f1); g.connect(f2); o.start(t); o.stop(t + dur + 0.05); });
...
function startCountdown() {
  notes = makeChart(); effects = []; faceBest = faceWorst = null; confetti = []; blinkStats = { both: 0, single: 0 };
...
function drawInner() {
  ...
  for (const n of notes) {
    const dt = n.t - songTime; if (dt > LEAD || dt < -1.4) continue;
    ...
    if (n.hit) {
      const m = Math.min(1, (songTime - n.hitAt) / 1.2);
      for (const l of lanes) {
        ...
        if (n.hit === 'perfect') { // swoon and topple sideways, face to the floor
          ...
          const pop = 1 + Math.sin(Math.min(1, m * 3) * Math.PI) * 0.18; // a quick swell before toppling
          if (!sprite(PEOPLE[who].down, x + dir * m * 26, hy + 70 + m * 10, { rot, scale: pop, alpha: 1 - Math.max(0, m - 0.8) * 5 })) { ctx.globalAlpha = 1 - m; ctx.fillText('😍', x, hy - m * 60); ctx.globalAlpha = 1; }
